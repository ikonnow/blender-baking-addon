...
- **Blender 3.3/3.6 (UDIM Init)**: 在旧版环境中，仅 `tiles.new(1001)` 可能不足以触发 C++ 层的缓冲区分配。
    - **避坑指南**: 必须执行“双重触发”：首先分配物理路径 (`image.filepath_raw`)，然后执行一次 `image.pixels.foreach_set` 触碰。否则烘焙算子会报 `Uninitialized image`，尤其在 Headless 模式下表现异常。
...
## 5. 开发者工作流：新增指标与参数一致性 (Standard Checklist)

为了防止因拼写错误或逻辑遗漏导致的 `AttributeError`（特别在跨版本环境下的元数据丢失），新增任何烘焙指标（如 `vram_usage`, `poly_count`）必须遵循以下 **“三点对齐”** 流程：

### 5.1 第一点：RNA 属性注册
在 `properties/props.py` 的 `BakedImageResult` 类中定义新属性。
```python
# properties/props.py
class BakedImageResult(PropertyGroup):
    vram_usage: FloatProperty(name="VRAM Usage", unit='MEMORY')
```

### 5.2 第二点：数据生产者 (Producer)
在 `core/engine.py` 的 `BakeStepRunner.run` 方法中计算并填入 `meta` 字典。
```python
# core/engine.py -> BakeStepRunner.run()
meta = {
    'vram_usage': 1024.5, # 获取实际数值
    ...
}
```

### 5.3 第三点：数据映射器 (Mapper)
在 `core/execution.py` 的 `add_bake_result_to_ui` 方法中手动映射。
```python
# core/execution.py -> add_bake_result_to_ui()
if meta:
    item.vram_usage = meta.get('vram_usage', 0.0)
```
