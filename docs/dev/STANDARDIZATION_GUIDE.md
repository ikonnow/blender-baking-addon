...
BakeTool 里有一部分 UI 不是直接写死字段名，而是通过映射、字符串拼接或 helper 生成属性路径、标题和标签。对于这类动态绑定，同样必须遵守参数同步原则：

- 不允许 UI 动态引用一个引擎根本不消费的字段
- 不允许属性层改名后，只修面板显示，不修执行链和测试
- 不允许“按钮能画出来”被误当成“参数已经接通”

### 2.4 默认值与回退逻辑的一致性 (Consistency of Fallbacks)

在处理动态通道或可选参数时，应确立“显式属性优先，安全回退兜底”的原则：
- **属性层**：在 `property.py` 中为 `default_value` 类属性设定最通用的默认值（通常为 0.0）。
- **执行层**：使用 `getattr(prop, "attr", fallback)` 时，`fallback` 应仅作为属性缺失（如旧版预设）时的补丁，而不应替代属性层逻辑。
- **UI层**：确保 UI 暴露的参数与引擎读取的属性路径完全对齐，严禁使用硬编码的临时变量替代属性引用。

标准做法是把来源尽量收敛到 `constants.py`、明确 helper 或集中映射中，并在 `suite_ui_logic` 或更贴近的套件里留下回归。
