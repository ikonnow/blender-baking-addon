...
BakeTool 遵循严格的 **UI-Engine-Core** 三层解耦原则：

```
┌─────────────────────────────────────────────────────────────┐
│                    UI / Operator 层                          │
│  ops.py, ui.py                                             │
│  - 数据驱动的 UI (CHANNEL_UI_LAYOUT)                        │
│  - 瘦 Operator (Thin Operators)                            │
│  - 环境健康监测                                            │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
...
│                    Engine 层 (编排器)                          │
│  core/engine.py                                            │
│  - JobPreparer: 验证输入并准备执行队列                      │
│  - BakePassExecutor: 执行烘焙步骤                          │
│  - BakeStepRunner: 异步烘焙主控                            │
│  - BakePostProcessor: 降噪后处理                           │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
...
def apply_baked_result(context, source_obj, images, setting, name):
    ...
    if existing in bpy.data.objects:
        ...
        # 创建新物体
        return create_new_object(existing, images)
```
...
def get_bake_settings(scene):
    """统一访问烘焙设置 (处理 5.0 的 render.bake vs 旧版本)"""
    if IS_BLENDER_5:
...
| suite_memory.py | 内存测试 | 内存泄漏检测 |
| suite_export.py | 导出测试 | 导出安全性 |
| suite_api.py | API 测试 | 公共 API 稳定性 |
| suite_ui_logic.py | UI 测试 | 面板绘制逻辑 |
| suite_preset.py | 预设测试 | 序列化与迁移 |
| suite_negative.py | 负面测试 | 边界条件 |
| suite_denoise.py | 降噪测试 | 降噪器集成 |
| suite_production_workflow.py | E2E 测试 | 端到端流程 |
| suite_context_lifecycle.py | 生命周期测试 | 上下文管理 |
| suite_parameter_matrix.py | 矩阵测试 | 参数组合测试 |
| suite_compat.py | 兼容性测试 | 版本兼容性 |
| suite_cleanup.py | 清理测试 | 资源清理 |
| suite_shading.py | 着色测试 | 着色器逻辑 |
| suite_udim_advanced.py | UDIM 测试 | UDIM 功能 |
| suite_code_review.py | 代码审查测试 | 静态检查 |
...
1. **Constants** → `constants.py` 定义所有系统常量
2. **Engine** → `core/engine.py` 使用常量
...
# 使用时
material = bpy.data.materials.get(obj["material_name"])
```
...
当 `items` 参数使用函数回调时，`default` **必须是整数索引**：
...
```python
# 旧版本
tree = bpy.data.node_groups.new("MyTree", type='COMPOSITING')
...
在 E2E 测试中，`assert_no_leak` 必须包裹所有生成逻辑：
