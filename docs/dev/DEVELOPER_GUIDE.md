...
### 5.2 稳健性准则：测试 Mock 策略 (v1.0.0 Hardened)
在编写单元测试（`suite_unit.py`）或负面测试（`suite_negative.py`）时，为防止因 Mock 对象不完整导致的 `AttributeError`，必须采取以下措施：
- **统一 MockSetting**: 优先使用 `test_cases/helpers.py` 中的 `MockSetting` 类。该类已预初始化了所有的核心属性（如 `res_x`, `bake_mode`, `color_depth` 等）。
- **动态探测**: 在核心逻辑中使用 `getattr(obj, 'attr', fallback)` 或 `hasattr` 检查，尤其是在处理 `NodeSocket` 的 `default_value` 时（不同版本的 Socket 可能返回 float 或 Vector）。
- **解耦 ExitStack**: 在 `NodeGraphHandler` 和 `BakeContextManager` 的 `__exit__` 中，必须执行幂等的清理操作。严禁假设 `__enter__` 已完全执行成功（例如捕获 `AttributeError` 后仍需恢复已备份的链接）。

### 5.3 故障预防与诊断 (Regresion Defense)
- **Constants 为准**: 所有的 attribute name 映射应当集中在 `constants.py` 或特定的 Mapping 字典中，严禁在 `engine.py` 中散落魔法字符串。
- **三重对齐验证**: `suite_parameter_matrix.py` 会自动对比 `BakeJobSetting` 的 RNA 属性与 `constants.BAKE_TYPES` 的映射，任何新增参数若未在该矩阵中注册，测试将直接报错。
...
- [x] **E2E Stability (v1.0.0)**: 100% matrix pass rate (70+ cases) for 3.3-5.0.
- [x] **Architecture Hardening**: Robust context manager (`ExitStack`), atomic link restoration, and fully decoupled image manager.
- [x] **Parameter Consistency Framework**: Triple-point alignment protocol established.
...
- [x] **Denoise Pipeline (v1.0.0)**: Hardened OIDN logic with zero-leak scene cleanup.
- [x] **Visual Preset Library (v0.9.3)**: Thumbnail gallery with dynamic refresh logic.
- [ ] **Background Process Baking (Phase v1.1.0)**: Detached worker process for 100% UI responsiveness.
- [ ] **External Engine Bridge (Phase v1.2.0)**: Unified API for Marmoset/Substance integration.
- [ ] **AI-Assisted Retopology Bridge (Phase v2.0.0)**: Automated LOD generation and smart baking.
