...
```
======================================================================
      BAKETOOL v1.5.1 VERIFICATION SUITE
======================================================================
...
1. **路径配置**: 将插件目录添加到 `sys.path`
2. **模块清理**: 清除已加载的插件模块
3. **环境输出**: 打印 Blender 版本和路径信息

### API

```python
from automation.env_setup import setup_environment

addon_name, addon_root = setup_environment()
print(f"Addon: {addon_name}")
print(f"Root: {addon_root}")
```
...
| 值 | 类型 | 描述 |
|-----|------|------|
| `addon_name` | str | 插件名称 (`"baketool"`) |
| `addon_root` | Path | 插件根目录 |
