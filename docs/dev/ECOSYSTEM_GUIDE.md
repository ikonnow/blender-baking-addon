...
└── docs/dev/
    ...
    └── STANDARDIZATION_GUIDE.md # 标准化指南
```
...
| **Headless CLI** | `blender -b --python` | CI/CD 自动化 |
...
│  - suite_unit.py      # 核心逻辑                            │
│  - suite_memory.py    # 内存泄漏检测                        │
│  - suite_export.py    # 导出安全性                          │
│  - suite_api.py       # API 稳定性                          │
│  - ...                                                  │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
...
│  - DataLeakChecker    # 数据泄漏检测                        │
│  - JobBuilder         # 流畅 API 构建测试任务               │
│  - MockSetting        # Mock 对象                           │
│  - cleanup_scene()     # 场景清理                           │
└─────────────────────────────────────────────────────────────┘
```
...
| 内存测试 | `suite_memory.py` | 内存泄漏检测 | memory |
| 导出测试 | `suite_export.py` | 导出安全性 | export |
| API 测试 | `suite_api.py` | 公共 API 稳定性 | core |
| UI 测试 | `suite_ui_logic.py` | 面板绘制逻辑 | ui |
| 预设测试 | `suite_preset.py` | 序列化与迁移 | core |
| 负面测试 | `suite_negative.py` | 边界条件 | core |
| 降噪测试 | `suite_denoise.py` | 降噪器集成 | core |
| 生产流测试 | `suite_production_workflow.py` | 端到端流程 | integration |
| 上下文生命周期 | `suite_context_lifecycle.py` | 上下文管理 | integration |
| 清理测试 | `suite_cleanup.py` | 资源清理 | core |
| 兼容性测试 | `suite_compat.py` | 版本兼容性 | core |
| 参数矩阵 | `suite_parameter_matrix.py` | 参数组合测试 | core |
| UDIM 高级 | `suite_udim_advanced.py` | UDIM 功能 | core |
| 着色测试 | `suite_shading.py` | 着色器逻辑 | core |
| 代码审查 | `suite_code_review.py` | 静态检查 | core |
...
| **multi_version_test.py** | `automation/` | 跨 Blender 版本测试 |
| **comprehensive_verification.py** | `automation/` | 修复验证 |
| **headless_bake.py** | `automation/` | 无头烘焙 CLI |
| **extract_translations.py** | `dev_tools/` | 翻译提取与同步 |
...
**SmartFilter 智能过滤：**
- ✅ 保留：用户可见文本 (`"Bake"`, `"Select Object"`)
- ❌ 忽略：内部 ID (`"BAKETOOL_OT_Bake"`)
- ❌ 忽略：数字 (`"1024"`, `"3.14"`)
- ❌ 忽略：单字符 (`"X"`, `"Y"`, `"Z"`)
...
- [ ] 运行所有测试套件 (`--suite all`)
- [ ] 运行跨版本测试 (`--verification`)
- [ ] 检查内存泄漏 (`suite_memory.py`)
- [ ] 验证导出安全性 (`suite_export.py`)
- [ ] 更新 `bl_info` 版本号
- [ ] 更新 CHANGELOG
- [ ] 运行翻译同步 (`--mode sync`)
...
*本指南由 BakeTool 团队维护 - 最后更新: 2026-04-17*
