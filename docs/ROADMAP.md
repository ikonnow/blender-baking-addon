...
BakeTool 是一款专为 Blender 设计的专业级纹理烘焙插件，支持 Blender 3.3 至 5.0+ 的所有主流版本。本路线图详细阐述了插件从实用工具向专业级"烘焙中间件"演进的战略愿景。

### 版本历史

| 版本 | 日期 | 状态 | 主要特性 |
|------|------|------|----------|
| 1.5.1 | 2026-04-17 | 当前版本 | 代码质量增强、异常处理硬化 |
| 1.5.0 | 2026-01-15 | 稳定版本 | Blender 5.0 支持、生产硬化 |
| 1.0.0 | 2024-06-01 | 稳定版本 | 交互预览、笼子分析 |
| 0.9.5 | 2024-01-20 | 历史版本 | GLB/USD 导出、降噪管线 |
...
**技术实现**:
- **三点点对齐协议**: Constants → Engine → Automation
...
- 支持 B5.0 统一节点系统（使用 `CompositorNodeTree` 和 `NodeGroupOutput`）
...
**工具链**:
- `multi_version_test.py`: 自动监控多个本地 Blender 安装的 70+ 核心测试套件
- 负面测试套件确保错误路径弹性
- 综合验证脚本 `comprehensive_verification.py`
...
**目标**: 探索基于 `bpy.app.timers` 的后台进度条优化。
