...
*   **Cage Analysis (笼子诊断) [NEW]**: 
    *   点击 **Analyze Cage**，系统将分析当前笼子设置是否会产生烘焙错误。
    *   **热力图演示**: 红色代表“碰撞/错过”，绿色代表“安全”。错误总数会直接显示在物体列表中。
*   **Texel Density (像素密度)**: 
...
*   **Zero-Friction Delivery (一键交付) [NEW]**:
    *   勾选 **Export Model** 并选择 `GLB` 或 `USD` 格式。
    *   **自动化流程**: 系统会在烘焙完成后，自动创建一个符合工业标准的 PBR 材质，并将其与模型一同导出。您无需手动连接任何贴图即可在 Substance 或引擎中查看最终效果。
*   **Path**: 输出目录。支持相对路径 `//`。
